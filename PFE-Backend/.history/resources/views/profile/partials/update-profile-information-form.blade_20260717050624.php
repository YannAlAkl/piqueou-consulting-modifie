    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="mt-4">
            <label class="flex items-center gap-3">
                <input id="wants_newsletter" name="wants_newsletter" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(old('wants_newsletter', $user->wants_newsletter)) />
                <span>{{ __('Subscribe to newsletter') }}</span>
            </label>
            <x-input-error class="mt-2" :messages="$errors->get('wants_newsletter')" />
        </div>
